<template>
    <div class="flex justify-between bg-[#AFB7F8]/31 rounded-lg py-4 px-4">
        <div class="ml-0.5 flex items-center gap-3.5">
            ...
            />
            <span
                class="text-sm font-medium text-slate-700 dark:text-orange-50"
                >{{ user }}</span
            >
        </div>
        <button
            class="flex items-center justify-center w-8 h-8 p-1 rounded-md cursor-pointer hover:bg-red-300 text-slate-700 dark:text-orange-50"
            @click="confirmOpen = true"
            :title="t('nav.profile.logout')"
        >
