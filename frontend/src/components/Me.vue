<template>
    <div class="p-4 flex flex-col items-center w-full max-w-3xl mx-auto">
        ...
        <div
            class="w-full bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-md shadow-black/5 flex flex-col items-center"
        >
            <h3
                class="text-sm font-semibold text-slate-700 dark:text-orange-50 tracking-wide mb-6 align-self-start w-full text-left"
            >
            ...
            >
                <div
                    class="w-64 h-64 shrink-0 flex items-center justify-center"
                >
                    <Pie :data="chartData" :options="chartOptions" />
                </div>

                <div
                    class="flex flex-col gap-2 text-sm text-slate-600 dark:text-slate-300 justify-center sm:pt-4 w-full"
                >
                    <div
...
const pieSlices = computed(() => {
    ...
    const filtered = timeEntryStore.myEntries.filter(
...
const timelineChartData = computed(() => {
    const projectId = currentProjectId.value;
    const filteredMyEntries = timeEntryStore.myEntries.filter(
        (e) => e.project?.id == projectId,
    );
    ...
    const durationData = labels.map((label) =>
        msToDecimalHours(dailyMap[label] || 0),
    );
    const finalLabels =
        timeframe.value === "month"
            ? labels.filter((_, idx) => durationData[idx] > 0)
            : labels;
    const finalData =
        timeframe.value === "month"
            ? durationData.filter((val) => val > 0)
            : durationData;
