<template>
    <div
        class="mt-5 w-full md:w-8/12 lg:w-6/12 bg-[#dde3ef] dark:bg-slate-600 rounded-2xl overflow-hidden shadow-md shadow-black/10"
    >
        ...
        <div class="bg-[#d4dae8] dark:bg-slate-500/40 px-6 py-4 space-y-5">
            <textarea
                :placeholder="t('nav.add.placeholder')"
                v-model="description"
                rows="4"
            ...
            >
                ...
                >
                    <label
                        class="text-sm text-slate-700 dark:text-orange-50 shrink-0"
                        >{{ t("nav.add.timeframe") }}</label
                    >
                    <div class="flex items-center gap-2">
                        ...
                        />
                        <span
                            class="text-slate-500 dark:text-orange-50 shrink-0"
                            >-</span
                        >
                        <input
                            type="time"
                ...
                <Button
                    ...
                    @click="save"
                    :disabled="saving || (touched && !!timeError)"
                >
                    <span class="dark:text-orange-50">{{
                        saving ? "..." : t("nav.add.label")
                    }}</span>
                </Button>
            </div>

            <p
                v-if="touched && timeError"
                class="text-sm text-red-500 dark:text-red-400"
            >
                {{ timeError }}
            </p>
            <p
                v-else-if="timeEntryStore.error"
                class="text-sm text-red-500 dark:text-red-400"
            >
