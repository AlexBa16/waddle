...
export const useRunningTimerStore = defineStore('runningTimer', () => {
    ...
    const elapsedSeconds = computed(() => {
        ...
        return Math.floor((Date.now() - new Date(startTime.value).getTime()) / 1000)
    })

    // Beim Store-Init: localStorage wiederherstellen
    function restore() {
    ...
    async function stop() {
        ...
        await timeEntryStore.save(projectId.value, description.value, startTime.value, endTime)
        reset()
    }
