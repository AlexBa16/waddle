<template>
    <div
        class="mt-5 w-6/12 bg-[#dde3ef] dark:bg-slate-600 rounded-2xl overflow-hidden shadow-md shadow-black/10"
    >
        <div class="flex justify-between gap-5 px-6 py-4">
            <div class="flex items-center gap-3 shrink-0">
                <div
                    class="flex items-center justify-center w-5 h-5 text-slate-700 dark:invert"
                >
                    <img
                        :src="iconPath"
                        alt="icon"
        ...
        <div class="bg-[#d4dae8] min-h-16">
            <template v-if="loading">
                ...
                >
                    <div
                        class="h-3 rounded-md bg-slate-300/70 animate-pulse"
                        style="width: 55%"
                    />
                    <div
                        class="h-3 rounded-md bg-slate-300/70 animate-pulse"
                    ...
                    />
                    <div
                        class="w-16 h-3 rounded-md bg-slate-300/70 animate-pulse"
                    />
                    <div />
                </div>
            </template>
            ...
            <div
                v-else-if="error"
                class="px-6 py-6 text-sm text-center text-red-500"
            >
            ...
            <TransitionGroup v-else name="row" tag="div">
                ...
                >
                    <span
                        class="text-sm font-semibold text-slate-800 dark:text-orange-50"
                        >{{ member.name }}</span
                    >
                    <span
                        class="font-mono text-xs text-slate-500 dark:text-orange-50/60"
                        >{{ member.email }}</span
                    >

                    <span
                        class="text-xs font-semibold px-2.5 py-1 rounded-lg w-fit"
                        :class="{
                            'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300':
                                member.isAdmin,
                            'bg-slate-200 text-slate-600 dark:bg-slate-600 dark:text-slate-300':
                                !member.isAdmin && !member.pending,
                    ...
                    <button
                        ...
                        :title="t('nav.projectSettings.deleteMember')"
                        @mouseover="hoveredId = member.id"
...
async function loadMembers() {
    ...
    error.value = null;
    try {
        members.value = await invitationStore.fetchMembers(
...
async function handleDelete(member) {
    ...
    try {
        await invitationStore.removeMember(
            projectStore.selected.id,
            member.invitationId,
        );
