        ...
        <Transition name="slide">
            <div v-if="isOpen || isDesktop"
                ...
                <div class="flex items-center justify-between mb-10">
                    ...
                    <button @click="isOpen = false"
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg"
                            class="w-5 h-5 cursor-pointer text-slate-700 dark:text-orange-50" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <nav class="flex flex-col gap-1 mt-4">
                    <RouterLink v-for="item in personalNavItems" :key="item.to" :to="item.to" @click="isOpen = false"
                        class="flex items-center gap-3 px-4 py-3 transition-colors rounded-xl text-slate-700 dark:text-orange-50 hover:bg-slate-300 dark:hover:bg-slate-500"
                        active-class="font-semibold bg-slate-300 dark:bg-slate-500">
                        <img :src="item.iconPathLight" alt="" class="block w-5 h-5 shrink-0 dark:hidden" />
                        <img :src="item.iconPathDark" alt="" class="hidden w-5 h-5 shrink-0 dark:block" />
