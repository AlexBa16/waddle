<template>
    <div class="w-full">
        ...
        <div
            v-else
            class="p-4 flex flex-col items-center w-full max-w-3xl mx-auto"
        >
            <div
            ...
            <div
                class="w-full bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-md shadow-black/5"
            >
                <h3
                ...
                <div
                    v-else
                    class="flex flex-col sm:flex-row gap-8 items-center sm:items-start w-full"
                >
                    <div class="flex-1 w-full space-y-3">
                        ...
                        >
                            ...
                            ></span>
                            <span
                                class="text-sm text-slate-600 dark:text-slate-300 flex-1 truncate"
                            >
                                {{ member.username }}
                            </span>
                            <span
                                class="text-sm font-semibold text-slate-700 dark:text-orange-50 tabular-nums"
                            >
                                {{ formatHours(member.ms) }}
                            </span>
                            <div
                                class="w-24 h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden"
                            >
                                <div
                                    ...
                                    :style="{
                                        width:
                                            totalProjectMs > 0
                                                ? `${(member.ms / totalProjectMs) * 100}%`
                                                : '0%',
                                        background:
                                            memberColors[
                                                idx % memberColors.length
                                            ],
                                    }"
                                ></div>
                            </div>
                    ...
                    <div
                        class="w-52 h-52 shrink-0 flex items-center justify-center"
                    >
...
const selectedMember = computed(() =>
    props.memberId
        ? (members.value.find((m) => m.id === props.memberId) ?? null)
        : null,
);
...
const timelineChartData = computed(() => {
    ...
    const datasets = memberStats.value.map((member, idx) => {
        const memberEntries = entries.filter(
            (e) => e.trackedBy?.id === member.userId,
        );
        const dailyMap = {};
    ...
    if (timeframe.value === "week") {
        ...
        const kwSet = new Set();
        datasets.forEach((ds) =>
            Object.keys(ds.map).forEach((k) => kwSet.add(k)),
        );
        labels = [...kwSet].sort(
            (a, b) =>
    ...
    datasets: [
        {
            ...
            backgroundColor: memberStats.value.map(
                (_, i) => memberColors[i % memberColors.length],
            ),
            borderColor: memberStats.value.map(
                (_, i) => memberColors[i % memberColors.length],
            ),
            borderWidth: 1,
        },
